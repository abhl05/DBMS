...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, E.SALARY, 'Higher Than Manager' AS TYPE
FROM EMPLOYEES E
JOIN EMPLOYEES M ON E.MANAGER_ID = M.EMPLOYEE_ID
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, E.SALARY, 'Above Dept Avg' AS TYPE
FROM EMPLOYEES E
WHERE E.SALARY > (
    SELECT AVG(SALARY)
    FROM EMPLOYEES E2
...
SELECT (E.FIRST_NAME || ' ' || E.LAST_NAME) AS FULL_NAME,
       ...
       D.DEPARTMENT_NAME,
       CASE
           WHEN E.SALARY > 1.5 * (SELECT AVG(SALARY) FROM EMPLOYEES WHERE DEPARTMENT_ID = E.DEPARTMENT_ID)
